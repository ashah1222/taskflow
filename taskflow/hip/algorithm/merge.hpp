...
/**
@file taskflow/hip/algorithm/merge.hpp
@brief hip merge algorithm include file
*/
...
struct hipMergePair {
  hipArray<T, N> keys;
  hipArray<unsigned, N> indices;
};

/** @private */
struct hipMergeRange {
  ...
  __device__ hipRange a_range() const {
    return hipRange { a_begin, a_end };
  }
  __device__ hipRange b_range() const {
    return hipRange { b_begin, b_end };
  }

  __device__ hipMergeRange to_local() const {
    return hipMergeRange { 0, a_count(), a_count(), total() };
  }

  // Partition from mp to the end.
  __device__ hipMergeRange partition(unsigned mp0, unsigned diag) const {
    return hipMergeRange { a_begin + mp0, a_end, b_begin + diag - mp0, b_end };
  }

  // Partition from mp0 to mp1.
  __device__ hipMergeRange partition(unsigned mp0, unsigned diag0,
    unsigned mp1, unsigned diag1) const {
    return hipMergeRange {
      a_begin + mp0,
      a_begin + mp1,
      b_begin + diag0 - mp0,
      b_begin + diag1 - mp1
...
/** @private */
template<
  hipMergeBoundType bounds = hipMergeBoundType::LOWER,
  typename a_keys_it, typename b_keys_it, typename comp_t
>
__device__ auto hip_merge_path(
  ...
  while(beg < end) {
    ...
    bool pred = (hipMergeBoundType::UPPER == bounds) ?
      comp(a_key, b_key) :
      !comp(b_key, a_key);
...
/** @private */
template<hipMergeBoundType bounds, typename keys_it, typename comp_t>
__device__ auto hip_merge_path(
  keys_it keys, hipMergeRange range, unsigned diag, comp_t comp
) {

  return hip_merge_path<bounds>(
...
/** @private */
template<hipMergeBoundType bounds, bool range_check, typename T, typename comp_t>
__device__ bool hip_merge_predicate(
  T a_key, T b_key, hipMergeRange range, comp_t comp
) {
  ...
  if(range_check && !range.a_valid()) {
    ...
    p = (hipMergeBoundType::UPPER == bounds) ? comp(a_key, b_key) :
                                               !comp(b_key, a_key);
  }
  return p;
}
...
/** @private */
inline __device__ auto hip_compute_merge_range(
  ...
  return hipMergeRange { mp0, mp1, diag0 - mp0, diag1 - mp1 };
}
...
__device__ auto hip_load_two_streams_reg(
  const T* a, unsigned a_count, const T* b, unsigned b_count, unsigned tid
) {

  b -= a_count;
  hipArray<T, vt> x;
  hip_strided_iterate<nt, vt>([&](auto i, auto index) {
...
__device__ void hip_load_two_streams_shared(A a, unsigned a_count,
  ...
  auto x = hip_load_two_streams_reg<nt, vt, T>(a, a_count, b, b_count, tid);
  hip_reg_to_shared_strided<nt>(x, tid, shared, sync);
}

/** @private */
template<unsigned nt, unsigned vt, typename T>
__device__ auto hip_gather_two_streams_strided(const T* a,
  unsigned a_count, const T* b, unsigned b_count, hipArray<unsigned, vt> indices,
  unsigned tid) {
  ...
  hipArray<T, vt> x;
  hip_strided_iterate<nt, vt>([&](auto i, auto j) {
...
std::enable_if_t<
  !(std::is_pointer<a_it>::value && std::is_pointer<b_it>::value),
  hipArray<T, vt>
> hip_gather_two_streams_strided(a_it a,
  unsigned a_count, b_it b, unsigned b_count, hipArray<unsigned, vt> indices, unsigned tid) {
...
__device__ void hip_transfer_two_streams_strided(
  a_it a, unsigned a_count, b_it b, unsigned b_count,
  hipArray<unsigned, vt> indices, unsigned tid, c_it c
) {

  using T = typename std::iterator_traits<a_it>::value_type;
  auto x = hip_gather_two_streams_strided<nt, vt, T>(
    a, a_count, b, b_count, indices, tid
  );

  hip_reg_to_mem_strided<nt>(x, tid, a_count + b_count, c);
}
...
*/
template<hipMergeBoundType bounds, unsigned vt, typename T, typename comp_t>
__device__ auto hip_serial_merge(
  const T* keys_shared, hipMergeRange range, comp_t comp, bool sync = true
) {

  auto a_key = keys_shared[range.a_begin];
  auto b_key = keys_shared[range.b_begin];

  hipMergePair<T, vt> merge_pair;
  hip_iterate<vt>([&](auto i) {
    bool p = hip_merge_predicate<bounds, true>(a_key, b_key, range, comp);
...
*/
template<hipMergeBoundType bounds,
...
__device__ auto block_merge_from_mem(
  a_it a, b_it b, hipMergeRange range_mem, unsigned tid, comp_t comp, T (&keys_shared)[S]
) {
  ...
  hip_load_two_streams_shared<nt, vt>(
  ...
  auto diag = vt * tid;
  auto mp = hip_merge_path<bounds>(keys_shared, range_local, diag, comp);
  ...
  auto merged = hip_serial_merge<bounds, vt>(
...
>
void hip_merge_path_partitions(
  P&& p,
  ...
  hip_kernel<<<B, nt, 0, p.stream()>>>([=]__device__(auto tid, auto bid) {
    auto range = hip_get_tile(bid, nt * vt, num_partitions);
    hip_strided_iterate<nt, vt>([=](auto, auto j) {
      auto index = range.begin + j;
      auto diag = min(spacing * index, a_count + b_count);
      buf[index] = hip_merge_path<bounds>(a, a_count, b, b_count, diag, comp);
    }, tid, range.count());
  });
}
...
void hip_merge_loop(
  ...
  auto has_values = !std::is_same<V, hipEmpty>::value;

  hip_merge_path_partitions<hipMergeBoundType::LOWER>(
...
*/
template <typename P>
unsigned hip_merge_buffer_size(unsigned a_count, unsigned b_count) {
  using E = std::decay_t<P>;
...
//  hipDeviceVector<std::byte> temp(hip_merge_buffer_size<P>(a_count, b_count));
//
//  detail::hip_merge_loop(
//    p,
...
//  hip_merge(
//    p,
//    a_keys_first, (const hipEmpty*)nullptr, a_keys_last,
//    b_keys_first, (const hipEmpty*)nullptr, b_keys_last,
//    c_keys_first, (hipEmpty*)nullptr, comp
//  );
//}
...
This function is equivalent to tf::hip_merge_by_key without values.
...
void hip_merge(
  ...
  hip_merge_by_key(
    p,
    a_keys_first, a_keys_last, (const hipEmpty*)nullptr,
    b_keys_first, b_keys_last, (const hipEmpty*)nullptr,
    c_keys_first, (hipEmpty*)nullptr, comp,
    buf
  );
}
...
// ----------------------------------------------------------------------------
// hipFlowCapturer merge algorithms
// ----------------------------------------------------------------------------

// Function: merge
template<typename A, typename B, typename C, typename Comp>
hipTask hipFlowCapturer::merge(
  A a_first, A a_last, B b_first, B b_last, C c_first, Comp comp
) {
  // TODO
  auto bufsz = hip_merge_buffer_size<hipDefaultExecutionPolicy>(
  ...
  (hipStream_t stream) mutable {
    hip_merge(hipDefaultExecutionPolicy{stream},
...
template<
  ...
  typename C
>
hipTask hipFlowCapturer::merge_by_key(
  a_keys_it a_keys_first, a_keys_it a_keys_last, a_vals_it a_vals_first,
  b_keys_it b_keys_first, b_keys_it b_keys_last, b_vals_it b_vals_first,
  c_keys_it c_keys_first, c_vals_it c_vals_first, C comp
) {
  ...
  return on([=, buf=MoC{hipDeviceVector<std::byte>(bufsz)}] 
  (hipStream_t stream) mutable {
    hip_merge_by_key(hipDefaultExecutionPolicy{stream},
      a_keys_first, a_keys_last, a_vals_first,
      b_keys_first, b_keys_last, b_vals_first,
      c_keys_first, c_vals_first,
      comp,
...
template<
  ...
  typename C
>
void hipFlowCapturer::merge_by_key(
  hipTask task,
  a_keys_it a_keys_first, a_keys_it a_keys_last, a_vals_it a_vals_first,
  b_keys_it b_keys_first, b_keys_it b_keys_last, b_vals_it b_vals_first,
  c_keys_it c_keys_first, c_vals_it c_vals_first, C comp
