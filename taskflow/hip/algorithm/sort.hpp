...
/**
@file taskflow/hip/algorithm/sort.hpp
@brief hip sort algorithm include file
*/
...
*/
constexpr int hip_find_log2(int x, bool round_up = false) {
  int a = 31 - hip_clz(x);
  if(round_up) {
    a += !is_pow2(x);
  }
...
__device__ auto hip_odd_even_sort(
  hipArray<T, vt> x, C comp, int flags = 0
) {
...
__device__ auto hip_odd_even_sort(
  hipKVArray<K, V, vt> x, C comp, int flags = 0
) {
  hip_iterate<vt>([&](auto I) {
    #pragma unroll
    for(auto i = 1 & I; i < vt - 1; i += 2) {
      if((0 == ((2<< i) & flags)) && comp(x.keys[i + 1], x.keys[i])) {
        hip_swap(x.keys[i], x.keys[i + 1]);
        hip_swap(x.vals[i], x.vals[i + 1]);
      }
    }
  });
  return x;
...
/** @private */
__device__ inline int hip_out_of_range_flags(int first, int vt, int count) {
...
/** @private */
__device__ inline auto hip_compute_merge_sort_frame(
...
/** @private */
__device__ inline auto hip_compute_merge_sort_range(
  unsigned count, unsigned partition, unsigned coop, unsigned spacing
) {

  auto frame = hip_compute_merge_sort_frame(partition, coop, spacing);
...
__device__ inline auto hip_compute_merge_sort_range(
  ...
  auto range = hip_compute_merge_sort_range(count, partition, coop, spacing);
...
struct hipBlockSort {
  ...
  static constexpr bool has_values = !std::is_same<V, hipEmpty>::value;
  ...
  static_assert(is_pow2(nt), "hipBlockSort requires pow2 number of threads");
  ...
  __device__ auto merge_pass(
    ...
    auto range = hip_compute_merge_sort_range(count, tid, coop, vt);
    unsigned diag = vt * tid - range.a_begin;

    // Store the keys into shared memory for searching.
    hip_reg_to_shared_thread<nt, vt>(x.keys, tid, storage.keys);

    // Search for the merge path for this thread within its list.
    auto mp = hip_merge_path<hipMergeBoundType::LOWER>(
      storage.keys, range, diag, comp
    );

    // Run a serial merge and return.
    auto merge = hip_serial_merge<hipMergeBoundType::LOWER, vt>(
    ...
    if(has_values) {
      ...
      hip_reg_to_shared_thread<nt, vt>(x.vals, tid, storage.vals);
      x.vals = hip_shared_gather<nt, vt>(storage.vals, merge.indices);
    }
  ...
  template<typename C>
  __device__ auto block_sort(hipKVArray<K, V, vt> x,
    ...
    if(count < nt * vt) {
      auto head_flags = hip_out_of_range_flags(vt * tid, vt, count);
      x = hip_odd_even_sort(x, comp, head_flags);
    } else {
      x = hip_odd_even_sort(x, comp);
    }
...
void hip_merge_sort_partitions(
  ...
  hip_kernel<<<B, nt, 0, p.stream()>>>([=] __device__ (auto tid, auto bid) {
    ...
    hip_strided_iterate<nt, vt>([=](auto, auto j) {
      ...
      auto range = hip_compute_merge_sort_range(count, index, coop, spacing);
      auto diag = min(spacing * index, count) - range.a_begin;
      buf[index] = hip_merge_path<hipMergeBoundType::LOWER>(
...
void merge_sort_loop(
  ...
  const bool has_values = !std::is_same<V, hipEmpty>::value;

  unsigned B = (count + E::nv - 1) / E::nv;
  unsigned R = hip_find_log2(B, true);
  ...
  //hipDeviceVector<K> keys_temp(R ? count : 0);
  //auto keys_output = keys_temp.data();
  ////std::cout << "keys_output = " << keys_temp.size()*sizeof(K) << std::endl;

  //hipDeviceVector<V> vals_temp((has_values && R) ? count : 0);
  ...
  hip_kernel<<<B, E::nt, 0, p.stream()>>>([=] __device__ (auto tid, auto bid) {
    ...
    using sort_t = hipBlockSort<E::nt, E::vt, K, V>;
    ...
    hipKVArray<K, V, E::vt> unsorted;
    unsorted.keys = hip_mem_to_reg_thread<E::nt, E::vt>(
      keys_input + tile.begin, tid, tile.count(), shared.keys
    );

    if(has_values) {
      unsorted.vals = hip_mem_to_reg_thread<E::nt, E::vt>(
  ...
  //hipDeviceVector<unsigned> mem(num_partitions);
  ...
  for(unsigned pass = 0; pass < R; ++pass) {
    ...
    hip_kernel<<<B, E::nt, 0, p.stream()>>>([=]__device__(auto tid, auto bid) {
      ...
      auto range = hip_compute_merge_sort_range(
...
@tparam K key type
@tparam V value type (default tf::hipEmpty)

@param count number of keys/values to sort

The function is used to allocate a buffer for calling tf::hip_sort.

*/
template <typename P, typename K, typename V = hipEmpty>
unsigned hip_sort_buffer_size(unsigned count) {
  ...
  unsigned B = (count + E::nv - 1) / E::nv;
  unsigned R = detail::hip_find_log2(B, true);
...
This method is equivalent to tf::hip_sort_by_key without values.

*/
template<typename P, typename K_it, typename C>
void hip_sort(P&& p, K_it k_first, K_it k_last, C comp, void* buf) {
  hip_sort_by_key(p, k_first, k_last, (hipEmpty*)nullptr, comp, buf);
}
...
hipTask hipFlow::sort(I first, I last, C comp) {
...
// Function: sort
template <typename I, typename C>
void hipFlow::sort(hipTask task, I first, I last, C comp) {
...
hipTask hipFlow::sort_by_key(K_it k_first, K_it k_last, V_it v_first, C comp) {
  return capture([=](hipFlowCapturer& cap){
    cap.make_optimizer<hipLinearCapturing>();
...
// Function: sort
template <typename I, typename C>
hipTask hipFlowCapturer::sort(I first, I last, C comp) {

  using K = typename std::iterator_traits<I>::value_type;

  auto bufsz = hip_sort_buffer_size<hipDefaultExecutionPolicy, K>(
  ...
  (hipStream_t stream) mutable {
    hip_sort(
      hipDefaultExecutionPolicy{stream}, first, last, comp, buf.get().data()
    );
  });
}
...
// Function: sort
template <typename I, typename C>
void hipFlowCapturer::sort(hipTask task, I first, I last, C comp) {
...
// Function: sort_by_key
template <typename K_it, typename V_it, typename C>
hipTask hipFlowCapturer::sort_by_key(
  ...
  auto bufsz = hip_sort_buffer_size<hipDefaultExecutionPolicy, K, V>(
    std::distance(k_first, k_last)
  );

  return on([=, buf=MoC{hipDeviceVector<std::byte>(bufsz)}] 
  (hipStream_t stream) mutable {
    hip_sort_by_key(hipDefaultExecutionPolicy{stream},
...
// Function: sort_by_key
template <typename K_it, typename V_it, typename C>
void hipFlowCapturer::sort_by_key(
  hipTask task, K_it k_first, K_it k_last, V_it v_first, C comp
) {
