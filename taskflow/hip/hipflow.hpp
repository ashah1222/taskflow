...
#include "hip_capturer.hpp"

/**
@file taskflow/hip/hipflow.hpp
@brief hipFlow include file
*/

namespace tf {

// ----------------------------------------------------------------------------
// class definition: hipFlow
// ----------------------------------------------------------------------------

/**
@class hipFlow

@brief class to create a %hipFlow task dependency graph

A %hipFlow is a high-level interface over hip Graph to perform GPU operations
using the task dependency graph model.
The class provides a set of methods for creating and launch different tasks
on one or multiple hip devices,
...
taskflow.emplace([&](tf::hipFlow& cf){
  ...
  tf::hipTask task1 = cf.kernel(grid1, block1, shm_size1, kernel1, args1);
  tf::hipTask task2 = cf.kernel(grid2, block2, shm_size2, kernel2, args2);
...
A %hipFlow is a task (tf::Task) created from tf::Taskflow
and will be run by @em one worker thread in the executor.
That is, the callable that describes a %hipFlow
will be executed sequentially.
Inside a %hipFlow task, different GPU tasks (tf::hipTask) may run
in parallel scheduled by the hip runtime.

Please refer to @ref GPUTaskinghipFlow for details.
*/
class hipFlow {
  ...
  public:
    ...
    /**
    @brief constructs a standalone %hipFlow

    A standalone %hipFlow does not go through any taskflow and
    ...
    */
    ...
    /**
    @brief destroys the %hipFlow and its associated native hip graph
    ...
    */
    ...
    /**
    @brief clears the %hipFlow object
    */
    void clear();

    /**
    @brief dumps the %hipFlow graph into a DOT format through an
           output stream
    */
    void dump(std::ostream& os) const;

    /**
    @brief dumps the native hip graph into a DOT format through an
           output stream

    The native hip graph may be different from the upper-level %hipFlow
    ...
    */
    ...
    A host task can only execute CPU-specific functions and cannot do any hip calls
    (e.g., @c hipMalloc).
    */
    template <typename C>
    hipTask host(C&& callable);

    /**
    @brief updates parameters of a host task

    The method is similar to tf::hipFlow::host but operates on a task
    of type tf::hipTaskType::HOST.
    */
    template <typename C>
    void host(hipTask task, C&& callable);
    ...
    */
    ...
    The method is similar to tf::hipFlow::kernel but operates on a task
    of type tf::hipTaskType::KERNEL.
    The kernel function name must NOT change.
    */
    template <typename F, typename... ArgsT>
    void kernel(
      hipTask task, dim3 g, dim3 b, size_t shm, F f, ArgsT&&... args
    );
    ...
    */
    hipTask memset(void* dst, int v, size_t count);

    /**
    @brief updates parameters of a memset task

    The method is similar to tf::hipFlow::memset but operates on a task
    of type tf::hipTaskType::MEMSET.
    ...
    */
    void memset(hipTask task, void* dst, int ch, size_t count);
    ...
    */
    hipTask memcpy(void* tgt, const void* src, size_t bytes);

    /**
    @brief updates parameters of a memcpy task

    The method is similar to tf::hipFlow::memcpy but operates on a task
    of type tf::hipTaskType::MEMCPY.
    The source/destination memory may have different address values but
    must be allocated from the same contexts as the original
    source/destination memory.
    */
    void memcpy(hipTask task, void* tgt, const void* src, size_t bytes);
    ...
    */
    ...
    >
    hipTask zero(T* dst, size_t count);

    /**
    @brief updates parameters of a memset task to a zero task

    The method is similar to tf::hipFlow::zero but operates on
    a task of type tf::hipTaskType::MEMSET.
    ...
    */
    template <typename T, std::enable_if_t<
      is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>* = nullptr
    >
    void zero(hipTask task, T* dst, size_t count);
    ...
    */
    template <typename T, std::enable_if_t<
      is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>* = nullptr
    >
    hipTask fill(T* dst, T value, size_t count);

    /**
    @brief updates parameters of a memset task to a fill task

    The method is similar to tf::hipFlow::fill but operates on a task
    of type tf::hipTaskType::MEMSET.
    ...
    */
    template <typename T, std::enable_if_t<
      is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>* = nullptr
    >
    void fill(hipTask task, T* dst, T value, size_t count);
    ...
    */
    ...
    The method is similar to tf::hipFlow::copy but operates on a task
    of type tf::hipTaskType::MEMCPY.
    The source/destination memory may have different address values but
    must be allocated from the same contexts as the original
    source/destination memory.
    */
    ...
    /**
    @brief offloads the %hipFlow onto a GPU and repeatedly runs it until
    ...
    Immediately offloads the present %hipFlow onto a GPU and
    repeatedly runs it until the predicate returns @c true.

    An offloaded %hipFlow forces the underlying graph to be instantiated.
    After the instantiation, you should not modify the graph topology
    but update node parameters.

    By default, if users do not offload the %hipFlow,
    ...
    */
    ...
    /**
    @brief offloads the %hipFlow and executes it by the given times
    ...
    */
    ...
    /**
    @brief offloads the %hipFlow and executes it once
    */
    void offload();
    ...
    */
    ...
    This method is similar to hipFlow::single_task but operates
    on an existing task.
    */
    template <typename C>
    ...
    */
    ...
    the task created from tf::hipFlow::for_each.
    */
    template <typename I, typename C>
    ...
    */
    ...
    /**
    @brief updates parameters of a kernel task created from
           tf::hipFlow::for_each_index

    The type of the iterators and the callable must be the same as
    the task created from tf::hipFlow::for_each_index.
    */
    template <typename I, typename C>
    void for_each_index(
    ...
    */
    ...
    /**
    @brief updates parameters of a kernel task created from
           tf::hipFlow::transform

    The type of the iterators and the callable must be the same as
    the task created from tf::hipFlow::for_each.
    */
    template <typename I, typename O, typename C>
    void transform(hipTask task, I first, I last, O output, C c);
    ...
    */
    ...
    hipTask reduce(I first, I last, T* result, B bop);
    ...
    the task created from tf::hipFlow::reduce.
    */
    template <typename I, typename T, typename C>
    void reduce(hipTask task, I first, I last, T* result, C op);

    /**
    @brief similar to tf::hipFlow::reduce but does not assume any initial
           value to reduce
    ...
    */
    template <typename I, typename T, typename B>
    hipTask uninitialized_reduce(I first, I last, T* result, B bop);

    /**
    @brief updates parameters of a kernel task created from
           tf::hipFlow::uninitialized_reduce

    The type of the iterators, result, and callable must be the same as
    the task created from tf::hipFlow::uninitialized_reduce.
    */
    template <typename I, typename T, typename C>
    ...
    */
    ...
    hipTask transform_reduce(I first, I last, T* result, B bop, U uop);

    /**
    @brief updates parameters of a kernel task created from
           tf::hipFlow::transform_reduce
    */
    template <typename I, typename T, typename B, typename U>
    void transform_reduce(hipTask, I first, I last, T* result, B bop, U uop);

    /**
    @brief similar to tf::hipFlow::transform_reduce but does not assume any initial
           value to reduce
    ...
    */
    template <typename I, typename T, typename B, typename U>
    hipTask transform_uninitialized_reduce(
      I first, I last, T* result, B bop, U uop
    );

    /**
    @brief updates parameters of a kernel task created from
           tf::hipFlow::transform_uninitialized_reduce
    */
    template <typename I, typename T, typename B, typename U>
    ...
    */
    ...
    @brief updates the parameters of a task created
           from tf::hipFlow::inclusive_scan

    This method is similar to tf::hipFlow::inclusive_scan
    ...
    */
    ...
    /**
    @brief similar to hipFlow::inclusive_scan but excludes the first value
    */
    template <typename I, typename O, typename C>
    ...
    @brief updates the parameters of a task created from
           tf::hipFlow::exclusive_scan

    This method is similar to tf::hipFlow::exclusive_scan
    but operates on an existing task.
    */
    template <typename I, typename O, typename C>
    ...
    /**
    @brief updates the parameters of a task created from
           tf::hipFlow::transform_inclusive_scan

    This method is similar to tf::hipFlow::transform_inclusive_scan
    but operates on an existing task.
    */
    template <typename I, typename O, typename B, typename U>
    ...
    /**
    @brief similar to hipFlow::transform_inclusive_scan but
    ...
    */
    ...
    /**
    @brief updates the parameters of a task created from
           tf::hipFlow::transform_exclusive_scan

    This method is similar to tf::hipFlow::transform_exclusive_scan
    but operates on an existing task.
    */
    template <typename I, typename O, typename B, typename U>
    ...
    This method is similar to tf::hipFlow::merge but operates on
    an existing task.
    */
    template <typename A, typename B, typename C, typename Comp>
    void merge(
    ...
    This method is similar to tf::hipFlow::sort but operates on
    an existing task.
    */
    template <typename I, typename C>
    ...
    */
    ...
    /**
    @brief updates the parameters of a task created from
           tf::hipFlow::sort_by_key

    This method is similar to tf::hipFlow::sort_by_key but operates on
    an existing task.
    */
    template <typename K_it, typename V_it, typename C>
    void sort_by_key(
    ...
    */
    ...
    /**
    @brief updates the parameters of a task created from
           tf::hipFlow::merge_by_key

    This method is similar to tf::hipFlow::merge_by_key but operates
    on an existing task.
    */
    template<
      typename a_keys_it, typename a_vals_it,
    ...
    */
    ...
    /**
    @brief constructs a subflow graph through tf::hipFlowCapturer
    ...
    A captured subflow forms a sub-graph to the %hipFlow and can be used to
    ...
    taskflow.emplace([&](tf::hipFlow& cf){
      ...
      tf::hipTask my_kernel = cf.kernel(my_arguments);

      // create a flow capturer to capture custom kernels
      tf::hipTask my_subflow = cf.capture([&](tf::hipFlowCapturer& capturer){
        capturer.on([&](hipStream_t stream){
    ...
    */
    template <typename C>
    hipTask capture(C&& callable);

    /**
    @brief updates the captured child graph

    The method is similar to tf::hipFlow::capture but operates on a task
    of type tf::hipTaskType::SUBFLOW.
    The new captured graph must be topologically identical to the original
    captured graph.
    */
    template <typename C>
    void capture(hipTask task, C callable);
...
// Construct a standalone hipFlow
inline hipFlow::hipFlow() :
  _handle {std::in_place_type_t<External>{}},
  _graph  {std::get_if<External>(&_handle)->graph} {

  TF_CHECK_HIP(
    hipGraphCreate(&_graph._native_handle, 0),
    "hipFlow failed to create a native graph (external mode)"
  );
}

// Construct the hipFlow from executor (internal graph)
inline hipFlow::hipFlow(hipGraph& g, Executor& executor) :
...
// Destructor
inline hipFlow::~hipFlow() {
  hipGraphDestroy(_graph._native_handle);
  _graph._native_handle = nullptr;
}

// Procedure: clear
inline void hipFlow::clear() {

  _exec.clear();

  TF_CHECK_HIP(
    hipGraphDestroy(_graph._native_handle), "failed to destroy native graph"
  );

  TF_CHECK_HIP(
    hipGraphCreate(&_graph._native_handle, 0), "failed to create native graph"
  );
...
// Function: empty
inline bool hipFlow::empty() const {
  return _graph._nodes.empty();
}

// Function: num_tasks
inline size_t hipFlow::num_tasks() const {
  return _graph._nodes.size();
}

// Procedure: dump
inline void hipFlow::dump(std::ostream& os) const {
  _graph.dump(os, nullptr, "");
}

// Procedure: dump
inline void hipFlow::dump_native_graph(std::ostream& os) const {
  hip_dump_graph(os, _graph._native_handle);
}
...
// Function: noop
inline hipTask hipFlow::noop() {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Empty>{}
  );
...
// Function: host
template <typename C>
hipTask hipFlow::host(C&& c) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Host>{}, std::forward<C>(c)
  );

  auto h = std::get_if<hipNode::Host>(&node->_handle);

  hipHostNodeParams p;
  p.fn = hipNode::Host::callback;
  p.userData = h;
...
// Function: kernel
template <typename F, typename... ArgsT>
hipTask hipFlow::kernel(
  dim3 g, dim3 b, size_t s, F f, ArgsT&&... args
) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Kernel>{}, (void*)f
  );
...
>
hipTask hipFlow::zero(T* dst, size_t count) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Memset>{}
  );

  auto p = hip_get_zero_parms(dst, count);
...
template <typename T, std::enable_if_t<
  is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>*
>
hipTask hipFlow::fill(T* dst, T value, size_t count) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Memset>{}
  );

  auto p = hip_get_fill_parms(dst, value, count);
...
>
hipTask hipFlow::copy(T* tgt, const T* src, size_t num) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Memcpy>{}
  );

  auto p = hip_get_copy_parms(tgt, src, num);
...
// Function: memset
inline hipTask hipFlow::memset(void* dst, int ch, size_t count) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Memset>{}
  );

  auto p = hip_get_memset_parms(dst, ch, count);
...
// Function: memcpy
inline hipTask hipFlow::memcpy(void* tgt, const void* src, size_t bytes) {

  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Memcpy>{}
  );

  auto p = hip_get_memcpy_parms(tgt, src, bytes);
...
// Function: host
template <typename C>
void hipFlow::host(hipTask task, C&& c) {

  if(task.type() != hipTaskType::HOST) {
    TF_THROW(task, " is not a host task");
  }

  auto h = std::get_if<hipNode::Host>(&task._node->_handle);
...
void hipFlow::kernel(
  ...
  if(task.type() != hipTaskType::KERNEL) {
  ...
  TF_CHECK_HIP(
    hipGraphExecKernelNodeSetParams(_exec, task._node->_native_handle, &p),
...
void hipFlow::copy(hipTask task, T* tgt, const T* src, size_t num) {

  if(task.type() != hipTaskType::MEMCPY) {
  ...
  TF_CHECK_HIP(
    hipGraphExecMemcpyNodeSetParams(_exec, task._node->_native_handle, &p),
    "failed to update memcpy parameters on ", task
  );
}

// Function: update memcpy parameters
inline void hipFlow::memcpy(
  hipTask task, void* tgt, const void* src, size_t bytes
) {
...
// Procedure: memset
inline void hipFlow::memset(hipTask task, void* dst, int ch, size_t count) {

  if(task.type() != hipTaskType::MEMSET) {
  ...
  TF_CHECK_HIP(
    hipGraphExecMemsetNodeSetParams(_exec, task._node->_native_handle, &p),
...
template <typename T, std::enable_if_t<
  is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>*
>
void hipFlow::fill(hipTask task, T* dst, T value, size_t count) {
...
template <typename T, std::enable_if_t<
  is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), void>*
>
void hipFlow::zero(hipTask task, T* dst, size_t count) {
...
// Function: capture
template <typename C>
void hipFlow::capture(hipTask task, C c) {

  if(task.type() != hipTaskType::SUBFLOW) {
    TF_THROW(task, " is not a subflow task");
  }

  // insert a subflow node
  // construct a captured flow from the callable
  auto node_handle = std::get_if<hipNode::Subflow>(&task._node->_handle);
  node_handle->graph.clear();

  hipFlowCapturer capturer(node_handle->graph);

  c(capturer);

  // obtain the optimized captured graph
  auto captured = capturer._capture();
  //hip_dump_graph(std::cout, captured);

  TF_CHECK_HIP(
    hipGraphExecChildGraphNodeSetParams(_exec, task._node->_native_handle, captured),
    "failed to update a captured child graph"
  );

  TF_CHECK_HIP(hipGraphDestroy(captured), "failed to destroy captured graph");
}
...
// Function: capture
template <typename C>
hipTask hipFlow::capture(C&& c) {

  // insert a subflow node
  auto node = _graph.emplace_back(
    _graph, std::in_place_type_t<hipNode::Subflow>{}
  );

  // construct a captured flow from the callable
  auto node_handle = std::get_if<hipNode::Subflow>(&node->_handle);
  node_handle->graph.clear();
  hipFlowCapturer capturer(node_handle->graph);
  ...
  TF_CHECK_HIP(
    ...
    "failed to add a hipFlow capturer task"
  );
...
template <typename P>
void hipFlow::offload_until(P&& predicate) {
...
template <typename P>
void hipFlow::_offload_until_external(P&& predicate) {
  if(!_exec) {
...
template <typename P>
void hipFlow::_offload_until_internal(P&& predicate) {
  ...
  hipStream stream;
  hipEvent event(hipEventDisableTiming);
...
// Procedure: offload_n
inline void hipFlow::offload_n(size_t n) {
  offload_until([repeat=n] () mutable { return repeat-- == 0; });
}

// Procedure: offload
inline void hipFlow::offload() {
...
template <typename C, typename D,
  std::enable_if_t<is_hipflow_task_v<C>, void>*
>
Task FlowBuilder::emplace_on(C&& c, D&& d) {
  auto n = _graph._emplace_back(
    std::in_place_type_t<Node::hipFlow>{},
    [c=std::forward<C>(c), d=std::forward<D>(d)] (Executor& e, Node* p) mutable {
      hipScopedDevice ctx(d);
      e._invoke_hipflow_task_entry(p, c);
    },
    std::make_unique<hipGraph>()
  );
  return Task(n);
}

// FlowBuilder::emplace
template <typename C, std::enable_if_t<is_hipflow_task_v<C>, void>*>
Task FlowBuilder::emplace(C&& c) {
  return emplace_on(std::forward<C>(c), tf::hip_get_device());
}
...
// Procedure: _invoke_hipflow_task_entry
template <typename C, std::enable_if_t<is_hipflow_task_v<C>, void>*>
void Executor::_invoke_hipflow_task_entry(Node* node, C&& c) {

  using T = std::conditional_t<
    std::is_invocable_r_v<void, C, hipFlow&>, hipFlow, hipFlowCapturer
  >;

  auto h = std::get_if<Node::hipFlow>(&node->_handle);

  hipGraph* g = dynamic_cast<hipGraph*>(h->graph.get());
  ...
  if(!(g->_state & hipGraph::OFFLOADED)) {
    cf.offload();
  }
}
