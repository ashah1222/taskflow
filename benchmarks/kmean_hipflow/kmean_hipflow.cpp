    ...
    const std::vector<float> &h_py)
{
    ...
    auto allocate_px = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_px, N*sizeof(float)), "failed to allocate d_px");
    }).name("allocate_px_gpu");
    auto allocate_py = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_py, N*sizeof(float)), "failed to allocate d_py");
    }).name("allocate_py_gpu");
    auto allocate_mx = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_mx, K*sizeof(float)), "failed to allocate d_mx");
    }).name("allocate_mx_gpu");
    auto allocate_my = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_my, K*sizeof(float)), "failed to allocate d_my");
    }).name("allocate_my_gpu");
    auto allocate_sx = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_sx, K*sizeof(float)), "failed to allocate d_sx"); 
    }).name("allocate_sx_gpu");
    auto allocate_sy = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_sy, K*sizeof(float)), "failed to allocate d_sy"); 
    }).name("allocate_sy_gpu");
    auto allocate_c = taskflow.emplace([&](){
        TF_CHECK_HIP(hipMalloc(&d_c, K*sizeof(float)), "failed to allocate d_c");
    }).name("allocate_c_gpu");

    // task for copying data from host to device
    auto h2d = taskflow.emplace([&](tf::hipFlow &cf){
    ...
    auto kmeans = taskflow.emplace([&](tf::hipFlow &cf){
        // init data on GPU
    ...
    auto stop = taskflow.emplace([&](tf::hipFlow &cf){
    ...
    auto free = taskflow.emplace([&](){
        TF_CHECK_HIP(hipFree(d_px), "failed to free d_px");
        TF_CHECK_HIP(hipFree(d_py), "failed to free d_py");
        TF_CHECK_HIP(hipFree(d_mx), "failed to free d_mx");
        TF_CHECK_HIP(hipFree(d_my), "failed to free d_my");
        TF_CHECK_HIP(hipFree(d_sx), "failed to free d_sx");
        TF_CHECK_HIP(hipFree(d_sy), "failed to free d_sy");
        TF_CHECK_HIP(hipFree(d_c),  "failed to free d_c");
...
int main(int argc, const char *argv[])
{
    ...
    if(argc != 4) {
        std::cerr << "usage: ./kmean_hipflow num_points k num_iterations\n";
