...
void taskflow(const Graph& g, unsigned num_cpus, unsigned num_gpus) {
  ...
  int* gy = nullptr;
  int* gz = nullptr;
  TF_CHECK_HIP(hipMallocManaged(&gx, N*sizeof(int)), "failed at hipMalloc");
  TF_CHECK_HIP(hipMallocManaged(&gy, N*sizeof(int)), "failed at hipMalloc");
  ...
  for(const auto& v : g.nodes) {
    ...
    if(v.g == -1) {
      ...
      tasks[v.v] = taskflow.emplace_on([&](tf::hipFlow& cf){
        ++counter;
